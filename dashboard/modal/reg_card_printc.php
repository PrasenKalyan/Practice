...
$opno = isset($_REQUEST['opno']) ? $_REQUEST['opno'] : '';
...
if ($a == 1 && $opno !== '') {
    ...
    if ($qry2) {
        ...
        $regno = isset($row['registerno']) ? $row['registerno'] : '';
        $regdate = isset($row['registerdate']) ? $row['registerdate'] : '';
        $regdate = isset($row['date']) ? $row['date'] : '';
        $opno1 = isset($row['op_no']) ? $row['op_no'] : '';
        $age = isset($row['age']) ? $row['age'] : '';
        $gender = isset($row['gender']) ? $row['gender'] : '';
        $tokenno = isset($row['tokenno']) ? $row['tokenno'] : '';
        $dnamek = isset($row['dname']) ? $row['dname'] : '';
        //$dname="Dr.".$row['doctorname'];
        $dname = isset($row['doctorname']) ? $row['doctorname'] : '';
        $regfee = isset($row['registerfee']) ? $row['registerfee'] : '';
        $vno = isset($row['visit_no']) ? $row['visit_no'] : '';
        $apptime = isset($row['appmnt_time']) ? $row['appmnt_time'] : '';
        $pname = isset($row['pname_type']) && isset($row['patientname']) ? $row['pname_type'] . "." . $row['patientname'] : '';
        $ctype = isset($row['pay_type']) ? $row['pay_type'] : '';
        $cardno = isset($row['card_no']) ? $row['card_no'] : '';
        $cons_fee = isset($row['cons_fee']) ? $row['cons_fee'] : '';
        $t = isset($row['tokenno']) ? $row['tokenno'] : '';
        $validity1 = isset($row['validity']) ? $row['validity'] : '';
        $validity = date('d/m/Y', strtotime($validity1));
        $auth_by = isset($row['auth_by']) ? $row['auth_by'] : '';
        $bill_num = isset($row['bill_num']) ? $row['bill_num'] : '';
        $hosp_fee = isset($row['hosp_fee']) ? $row['hosp_fee'] : '';
        $serv_name = isset($row['serv_name']) ? $row['serv_name'] : '';
        $mobile = isset($row['phoneno']) ? $row['phoneno'] : '';
        $ref_doc = isset($row['ref_doc']) ? $row['ref_doc'] : '';
        $opt_type = isset($row['opt_type']) ? $row['opt_type'] : '';
    }
}
...
$ref_docname = isset($row1['ref_docname']) ? $row1['ref_docname'] : '';
$dd1 = "SELECT * FROM `doctdept` WHERE id = '$dnamek'";
$docid1 = mysqli_query($link, $dd1);

$row1 = mysqli_fetch_array($docid1);
$doctdeptname = isset($row1['doctdeptname']) ? $row1['doctdeptname'] : '';
...
<tr  style=" height:10px; background-color:#213; color:#FFF;"><td colspan="5"> <h1 align="center" style="font-size:18px;"><b><u>OP Cancellation Bill</u></b></h1></td></tr>
  </THEAD>
   <TBODY>
...
<tr>
	...
	<fieldset style="border='1px solid;   '">
        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#FFFFFF;"> <!--style="border:1px #BDD9E1 solid">-->
         ...
         <?php $sqqq=mysqli_query($link,"select * from daily_amount where mrnum='$regno' and amnt_desc='Patient Register Canceled' ");
         ...
         <tr><td align="center" style=" border-top: #000000 0px solid; width:100%" colspan="10">
		     <?php $sq=mysqli_query($link,"select * from op_cancel where mrno='$regno' ");
		 $z=mysqli_fetch_array($sq);
     $rem=$z['rem'];
     ?>
		 <table width="99%" cellpadding="0" cellspacing="0" border='0' ><tr><td>
		<table style="border:1px solid; width:100%"><tr>
    <td><b>Sno</b></td><td><b> Remarks</b></td><td><b>Details</b></td><td><b>Amount</b></td></tr>
<tr><td colspan='1'><hr></td></tr>

    <tr><td><b>1</b></td><td><b><?php echo $rem?></b></td><td><b>Cancellation</b></td><td><b><?php echo $tot?></b></td></tr>
	
	<tr><td colspan='3'><hr></td></tr>
	
	<tr><td></td><td></td>
	
	<td colspan="0"  ><span style="margin-left:-70px">Total Return Amount:</span>  <?php echo $amount?></td></tr>
	...
	<td><span style="margin-left:-45px">Balance:</span>  <?php echo $amount-$amount;?></td></tr>

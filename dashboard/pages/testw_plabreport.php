...
$i=1;
          ...
          $x="select * from bill1_proc where  testname='$tname' and 
  cdate between '$sdate' and '$edate' ";
$qry1=mysqli_query($link,$x);
while($r=mysqli_fetch_array($qry1)){
        $mrno=$r['mrno'];
        $bno=$r['bno'];
        $k=mysqli_query($link,"select * from bill_proc where billno='$bno' and mrno='$mrno'") or die(mysqli_error($link));
        ...
        <?php }else{ 
        $kj=mysqli_query($link,"select distinct testname from bill1_proc where cdate between '$sdate' and '$edate' ");
...
$i=1;
          ...
          $x="select * from bill1_proc where  testname='$testname' and 
  cdate between '$sdate' and '$edate' ";
$qry1=mysqli_query($link,$x);
$t=0;
while($r=mysqli_fetch_array($qry1)){

...
include('../db/connection.php');
 $sql='select distinct registerno from patientregister as a,`op_pat_dlt` as b where a.registerno=b.pat_regno order by a.reg_id desc '; $r=mysqli_query($link,$sql) or die(mysqli_error());
...
<?php 
             ...
             <tr height="25px"><td style="text-align:center;"><input type="radio" name="empid" value="<?php echo $r ?>" onclick="javascript:showDoc(this.value);"/></td><td><?php echo $r ?></td><td><?php echo $h;?></td></tr>
				 ...
				 // $x="select distinct a.PAT_NO,b.patientname,b.registerno from ip_pat_admit as a,patientregister as b
				  ...
				  $x="select distinct PAT_REGNO,patientname,a.registerno from patientregister as a,`op_pat_dlt` as b where a.registerno=b.pat_regno order by a.reg_id desc";

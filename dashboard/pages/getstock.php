...
$prdnames = $_REQUEST['prdnames'];
	...
	if($rtype == "a"){
		if($reporttype == "All"){
			...
			$query=mysqli_query($link,"select a.product_code,a.product_name,a.packing_type,a.mrp,a.balance_qty,a.batch_no,a.exp_date,a.noitems from phr_purinv_dtl as a,phr_prddetails_mast as b where b.type='$prdtype' and a.product_code=b.PRD_code and a.balance_qty<>0");
		} 
		else if(reporttype.equals("All2")){
			$query=mysqli_query($link,"select a.product_code,a.product_name,a.packing_type,a.mrp,a.balance_qty,a.batch_no,a.exp_date,a.noitems from phr_purinv_dtl as a,phr_prddetails_mast as b where a.product_code='$prdnames' and a.product_code=b.PRD_code and a.balance_qty<>0");
		}
		else{
			$fdate = date('Y-m-d',strtotime($_REQUEST['fdate']));
		$tdate = date('Y-m-d',strtotime($_REQUEST['tdate']));
			
			$query=mysqli_query($link,"select a.product_code,a.product_name,a.packing_type,a.mrp,a.balance_qty,a.batch_no,a.exp_date,a.noitems from phr_purinv_dtl as a,phr_prddetails_mast as b where b.type='$prdtype' and a.product_code=b.PRD_code AND a.currentdate between '$fdate' and '$tdate' and a.balance_qty<>0");
		}
		...
		$tdate = date('Y-m-d',strtotime($_REQUEST['tdate']));
			 ...
			 $query=mysqli_query($link,"select a.product_code,a.product_name,a.packing_type,a.mrp,a.balance_qty,a.batch_no,a.exp_date,a.noitems from phr_purinv_dtl as a,phr_prddetails_mast as b where b.type='$prdtype' and a.product_code=b.PRD_code AND a.exp_date between '$fdate' and '$tdate' and a.balance_qty<>0");
		}
	}
	else{
	if($reporttype == "All"){

...
$validation = new Validation();
			...
			$dt3 = ltrim($dt2, '0');
			$str20 = "BL-".$dt3."-";
			$tp="select count(distinct billno) as billcont from bill_proc where billno like '$str20%'";
			$rst = $crud->getdata($tp);
...
date_default_timezone_set('Asia/Kolkata');
 $ndate=date( 'Y-m-d H:i:s', time ()); 
	$aa="insert into daily_amount(amnt_type,amount,bill_num,mrnum,recv_by,pay_type,date_time,amnt_desc)
	 values('Procedure Lab Bill','$pamount','$cnt','$mrno','$user','$paymenttype','$ndate','Procedure Lab Bill')";
	...
	$sql = $crud->execute($aa);
		...
		$ts="INSERT INTO bill_proc(mrno,billno,bdate,pname,age,gender,dname,mobile,ptype,time,tamount,discount,namount,pamount,bamount,remarks,user,paymenttype,reg_no)VALUES
		...
		$amount= $_POST['amount'][$t];
			if($tname!=''){
				 $ts1="insert into bill1_proc(bno,mrno,testname,amount,cdate)values('$bno','$mrno','$tname','$amount','$bdate')";
	...
	$bno = ($_POST['bno']);
		...
		if($result1){
			...
			print "self.location='print_labbillk.php?id=$bno';";
			print "</script>";
		}
	}	

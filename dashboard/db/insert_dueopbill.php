...
$id=$crud->my_simple_crypt($_REQUEST['id'],'d');
			...
			$tp="select *  from opbill where billno='$id'";
			$rst = $crud->getdata($tp);
...
$mrno = $crud->escape_string($_POST['mrno']);
	...
	$rec_no= $crud->escape_string($_POST['rec_no']);
...
date_default_timezone_set('Asia/Kolkata');
 $ndate=date( 'Y-m-d H:i:s', time ()); 
	$aa="insert into daily_amount(amnt_type,amount,bill_num,mrnum,recv_by,pay_type,date_time,amnt_desc)
	 values('Daycare Due Bill','$damount','$cnt','$mrno','$user','$paymenttype','$ndate','Daycare Due Bill')";
	mysqli_query($link,$aa);
	
	
	
	
		  $ts="update opbill set mrno='$mrno',billno='$bno',bdate='$bdate',pname='$pname',age='$age',gender='$gender',dname='$dname',mobile='$mobile',ptype='$ptype',time='$time',tamount='$tamount',discount='$discount',namount='$namount',pamount='$paidamount',bamount='$ramount',remarks='$remarks',user='$user',paymenttype='$paymenttype',rec_no='$rec_no' where id='$id'";
		...
		$amount= $_POST['amount'][$t];
		 $id1= $_POST['id1'][$t];
			if($tname!='' and $id1!=''){
				  $ts1="update opbill1 set testname='$tname',amount='$amount' where id1='$id1' and mrno='$mrno'";
				$result1 = $crud->execute($ts1);
			}else{
				if($tname!='' and $id1==''){
			  	$ts2="insert into opbill1(bno,mrno,testname,amount,cdate)values('$bno','$mrno','$tname','$amount','$bdate')";
				$crud->execute($ts2);
				}
			}
	...
	//exit;
		if($result){
			...
			print "self.location='opbilllist.php';";
			print "</script>";
		}
	}	

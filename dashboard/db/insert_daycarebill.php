...
$validation = new Validation();
			...
			$dt3 = ltrim($dt2, '0');
			$str20 = "BL-".$dt3."-";
			$tp="select count(distinct billno) as billcont from daycarebill where billno like '$str20%'";
			$rst = $crud->getdata($tp);
...
date_default_timezone_set('Asia/Kolkata');
 $ndate=date( 'Y-m-d H:i:s', time ()); 
	$aa="insert into daily_amount(amnt_type,amount,bill_num,mrnum,recv_by,pay_type,date_time,amnt_desc)
	 values('Daycare Bill','$pamount','$cnt','$mrno','$user','$paymenttype','$ndate','Daycare Bill')";
	...
	$sql = $crud->execute($aa);
		...
		$ts="INSERT INTO daycarebill(mrno,billno,bdate,pname,age,gender,dname,mobile,ptype,time,tamount,discount,namount,pamount,bamount,remarks,user,paymenttype)VALUES
		...
		$amount= $_POST['amount'][$t];
			if($tname!=''){
				 $ts1="insert into daycarebill1(bno,mrno,testname,amount,cdate)values('$bno','$mrno','$tname','$amount','$bdate')";
	...
	$bno = $crud->my_simple_crypt($_POST['bno'],'e');
		...
		if($result1){
			...
			print "self.location='print_daycare.php?id=$bno';";
			print "</script>";
		}
	}	

...
$id=$crud->my_simple_crypt($_REQUEST['id'],'d');
			...
			$tp="select *  from gpbill where billno='$id'";
			$rst = $crud->getdata($tp);
...
date_default_timezone_set('Asia/Kolkata');
 $ndate=date( 'Y-m-d H:i:s', time ()); 
	...
	mysqli_query($link,$aa);
		  ...
		  $ts="update gpbill set mrno='$mrno',billno='$bno',bdate='$bdate',pname='$pname',age='$age',gender='$gender',dname='$dname',mobile='$mobile',ptype='$ptype',time='$time',tamount='$tamount',discount='$discount',namount='$namount',pamount='$paidamount',bamount='$ramount',remarks='$remarks',user='$user',paymenttype='$paymenttype' where id='$id'";
		...
		$amount= $_POST['amount'][$t];
		 $id1= $_POST['id1'][$t];
			if($tname!='' and $id1!=''){
				  $ts1="update gpbill1 set testname='$tname',amount='$amount' where id1='$id1' and mrno='$mrno'";
				$result1 = $crud->execute($ts1);
			}else{
				if($tname!='' and $id1==''){
			  	$ts2="insert into gpbill1(bno,mrno,testname,amount,cdate)values('$bno','$mrno','$tname','$amount','$bdate')";
				$crud->execute($ts2);
				}
			}
	...
	//exit;
		if($result){
			...
			print "self.location='gpbilllist.php';";
			print "</script>";
		}
	}	
